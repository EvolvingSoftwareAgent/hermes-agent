...
def test_config_bridges_whatsapp_lane_settings(monkeypatch, tmp_path):
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        "whatsapp:\n"
        "  enable_lanes: true\n"
        "  lane_idle_minutes: 30\n"
        "  lane_max_live_per_chat: 3\n",
        encoding="utf-8",
    )
    ...
    assert extra["enable_lanes"] is True
    assert extra["lane_idle_minutes"] == 30
    assert extra["lane_max_live_per_chat"] == 3
